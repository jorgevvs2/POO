...
void HugeInteger::operator=(string num){
    input(num);
}

void HugeInteger::setSize(int x){
    size = x;
}

HugeInteger HugeInteger::operator+(HugeInteger num2){
    int greater = size > num2.size ? size: num2.size;
    int sum, rest = 0;
    HugeInteger aux;
    aux.setSize(greater);
    ...
    if(rest == 1){
        aux.size ++;
        aux.number[size - 1] = '0' + 1;
    }

    return aux;
}

void HugeInteger::operator+=(HugeInteger num2){
    add(num2);
}

bool HugeInteger::operator==(HugeInteger num2){
    return isEqualTo(num2);
}

bool HugeInteger::operator!=(HugeInteger num2){
    return isNotEqualTo(num2);
}
...
bool HugeInteger::operator>=(HugeInteger num2){
    return isGreaterThanOrEqual(num2);
}

bool HugeInteger::operator<=(HugeInteger num2){
    return isLessThanOrEqual(num2);
}